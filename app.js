...
function openModal(product){
  const dialog=$('#productModal');let selectedSize='M',selectedType='Fan',selectedPatch='Sin parche',photoIndex=0,guideOpen=false,selectedName='',selectedNumber='';
  ...
  const personalization=`<fieldset class="personalization"><legend>Personalización (opcional)</legend><div class="personalization-fields"><label>Nombre<input id="customName" type="text" placeholder="Ej: MARIO" maxlength="20" autocomplete="off"></label><label>Dorsal<input id="customNumber" type="text" placeholder="Ej: 7" maxlength="2" inputmode="numeric" autocomplete="off"></label></div><p class="personalization-hint">Ejemplo: <strong>MARIO 7</strong> — primero el nombre y luego el dorsal. La personalización (nombre y dorsal) y los parches no tienen coste extra: el precio solo depende del tipo de camiseta (Fan, Jugador o Retro).</p></fieldset>`;
  ...
  const bindModal=()=>{
    ...
    if($('#customName')){$('#customName').oninput=()=>{selectedName=$('#customName').value.trim().toUpperCase()};$('#customName').value=selectedName}
    if($('#customNumber')){$('#customNumber').oninput=()=>{selectedNumber=$('#customNumber').value.trim()};$('#customNumber').value=selectedNumber}
    $('#consultInstagram').onclick=()=>{const type=product.isRetro?'Retro':selectedType;const personalizationLine=selectedName||selectedNumber?`${selectedName||'-'} ${selectedNumber||'-'}`.trim():'Sin personalizar';const message=`Hola FutGonZ, quiero consultar esta camiseta:\n\nProducto: ${product.name}\nReferencia: ${product.id}\nCategoria: ${product.category}\nEquipo: ${product.team}\nTemporada: ${product.season}\nTipo: ${type}\nTalla: ${selectedSize}\nNombre y dorsal: ${personalizationLine}\nParche: ${selectedPatch}\nPrecio: ${money(priceFor(product,selectedType))}\n\nLa teneis disponible?`;copyText(message).then(()=>toast('Pedido copiado. Pegalo en el chat de Instagram y envialo.'))};
...
['search','typeFilter','categoryFilter','leagueFilter','teamFilter','seasonFilter','brandFilter','stockFilter'].forEach(id=>$('#'+id).addEventListener('input',()=>{visibleProducts=20;render()}));
